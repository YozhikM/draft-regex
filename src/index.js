...
const setSelectionState = (key: string, start: number = 0, end?: number): SelectionState => {
...
const prepareOptionableRules = (options: Options): Array<Rule> => {
...
export const replaceTextRegex = (editorState: EditorState, options?: Options): EditorState => {
  ...
  if (match) {
    ...
    const newBlock = onChangeBlock(currentContentBlock, {
      text,
      key,
    });

    const diffInLengths = currentContentBlock.getLength() - newBlock.getLength();
    const newSS = setSelectionState(key, anchorOffset - diffInLengths);
    const copyCS = CS.getBlocksAsArray().slice();

    copyCS.forEach((block, i) => {
      if (block === currentContentBlock) {
        copyCS.splice(i, 1, newBlock);
      }
    });

    const newCS = ContentState.createFromBlockArray(copyCS);
